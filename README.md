...
上传datasets文件夹，RNP.cpp, RNPfunc_old.h, RNPfunc_bit.h, loaddata_char.h, loaddata_int.h, type_bit.h, type_old.h。
_old表示按照原有论文复现，_bit表示尝试修改为二进制位图实现。（二进制位图实现并不完整，可以在编译时排除_bit的文件）
我的环境是vs2022，需要在与RNP.cpp同一级的目录下，有datasets文件夹。



2026/4/29 上传cpp文件夹，这是第二种复现，代码是纯ai生成的
